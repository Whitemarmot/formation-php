            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="subtitle" id="subtitle" value="{{ old('subtitle', $formation->subtitle) }}"
                    ...
                    <div>
                        ...
                        <select name="level" id="level" required
                                class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-accent focus:border-accent">
                            <option value="1" {{ old('level', $formation->level) == 1 ? 'selected' : '' }}>Niveau 1 - Debutant</option>
                            <option value="2" {{ old('level', $formation->level) == 2 ? 'selected' : '' }}>Niveau 2 - Intermediaire</option>
                            <option value="3" {{ old('level', $formation->level) == 3 ? 'selected' : '' }}>Niveau 3 - Expert</option>
                        </select>
                    </div>

                    <div>
                        <label for="page_count" class="block text-sm font-medium text-gray-700 mb-1">Nombre de pages</label>
                        <input type="number" name="page_count" id="page_count" value="{{ old('page_count', $formation->page_count) }}" min="0"
                               class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-accent focus:border-accent">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="short_description" id="short_description" rows="2"
                                  class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-accent focus:border-accent">{{ old('short_description', $formation->short_description) }}</textarea>
                    </div>

                    <div class="md:col-span-2">
                        <label for="description" class="block text-sm font-medium text-gray-700 mb-1">Description complete *</label>
                        <textarea name="description" id="description" rows="6" required
                                  class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-accent focus:border-accent @error('description') border-red-500 @enderror">{{ old('description', $formation->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price', $formation->price) }}" step="0.01" min="0" required
                    ...
                    <div>
                        ...
                        <input type="number" name="sale_price" id="sale_price" value="{{ old('sale_price', $formation->sale_price) }}" step="0.01" min="0"
                               class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-accent focus:border-accent">
                    </div>

                    <div>
                        <label for="sort_order" class="block text-sm font-medium text-gray-700 mb-1">Ordre d'affichage</label>
                        <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $formation->sort_order) }}" min="0"
                               class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-accent focus:border-accent">
                    </div>
                </div>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        @if($formation->cover_image)
                            <div class="mb-2">
                                <img src="{{ Storage::url($formation->cover_image) }}" alt="Couverture actuelle" class="w-24 h-24 object-cover rounded">
                            </div>
                        @endif
                    ...
                    <div>
                        ...
                        @if($formation->pdf_path)
                            <p class="mb-2 text-sm text-green-600"><i class="fas fa-check-circle mr-1"></i>PDF present</p>
                        @endif
                        <input type="file" name="pdf_file" id="pdf_file" accept=".pdf"
                               class="w-full border border-gray-300 rounded-lg p-2">
                        <p class="mt-1 text-xs text-gray-500">Laisser vide pour conserver le PDF actuel</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-4">
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $formation->is_active) ? 'checked' : '' }}
                               class="rounded border-gray-300 text-accent focus:ring-accent">
                        <span class="ml-2 text-sm text-gray-700">Formation active (visible sur le site)</span>
                    </label>

                    <label class="flex items-center">
                        <input type="checkbox" name="is_featured" value="1" {{ old('is_featured', $formation->is_featured) ? 'checked' : '' }}
                               class="rounded border-gray-300 text-accent focus:ring-accent">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="meta_title" id="meta_title" value="{{ old('meta_title', $formation->meta_title) }}" maxlength="255"
                               class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-accent focus:border-accent">
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="meta_description" id="meta_description" rows="2" maxlength="500"
                                  class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-accent focus:border-accent">{{ old('meta_description', $formation->meta_description) }}</textarea>
                    </div>
                </div>
            </div>

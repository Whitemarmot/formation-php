<x-app-layout>
    <x-slot name="title">Formations Soudage par Points | Mang-Ky Ha</x-slot>
            ...
            <div class="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                @foreach($formations as $formation)
                    <div class="card card-hover flex flex-col">
                        <!-- Level Badge -->
                        <div class="p-6 pb-0">
                            <span class="level-badge level-{{ $formation->level }}">
                                Niveau {{ $formation->level }} - {{ $formation->level_label }}
                            </span>
                        </div>
                        ...
                        <div class="p-6 flex-1">
                            ...
                            <p class="text-gray-400 mb-4">{{ $formation->subtitle }}</p>
                            <p class="text-sm text-gray-500 mb-6">{{ $formation->short_description }}</p>
                            ...
                            <div class="flex items-center justify-between mb-4">
                                <div class="price-tag">
                                    @if($formation->is_on_sale)
                                        <span class="price-old">{{ $formation->formatted_price }}</span>
                                    @endif
                                    <span class="price">{{ $formation->formatted_current_price }}</span>
                                </div>
                            </div>
                            <div class="flex gap-2">
                                <a href="{{ route('formations.show', $formation) }}" class="btn btn-secondary flex-1">
                                    Voir détails
                                </a>
                                <form action="{{ route('cart.add', $formation) }}" method="POST" class="flex-1">
                                    @csrf
                ...
                <div class="card border-accent/50 overflow-hidden">
                    <div class="bg-gradient-to-r from-accent/20 to-accent-orange/20 p-8 text-center">
                        ...
                        <p class="text-gray-400">
                            Économisez {{ number_format($bundleRegularPrice - $bundlePrice, 2, ',', ' ') }} € avec le pack complet
                        </p>
                    </div>
                    ...
                    <div class="p-8">
                        <div class="grid md:grid-cols-2 gap-8">
                                ...
                                <ul class="space-y-3">
                                    @foreach($formations as $formation)
                                        <li class="flex items-center gap-3">
                                            ...
                                            <span class="text-gray-300">{{ $formation->name }}</span>
                                        </li>
                                    @endforeach
                                    <li class="flex items-center gap-3">
                                        <svg class="w-5 h-5 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <div class="text-center md:text-right">
                                <div class="mb-4">
                                    <span class="text-gray-500 line-through text-xl">{{ number_format($bundleRegularPrice, 2, ',', ' ') }} €</span>
            ...
            <div class="max-w-5xl mx-auto">
                <div class="grid md:grid-cols-2 gap-12 items-center">
                    ...
                    <div>
                        ...
                        <a href="{{ route('formateur') }}" class="btn btn-secondary">
                            En savoir plus
